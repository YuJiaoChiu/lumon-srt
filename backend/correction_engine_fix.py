...
class CorrectionEngine:
    ...
    def correct_subtitles(self, text: str, callback=None) -> Tuple[str, Dict[Tuple[str, str], int]]:
        ...
        # Create a case-insensitive dictionary for lookups
        case_insensitive_dict = {}
        for wrong, correct in self.correction_dict.items():
            wrong_lower = wrong.lower()
            if wrong_lower not in case_insensitive_dict:
                case_insensitive_dict[wrong_lower] = correct

        # Precompile patterns for better performance
        patterns = {}
        for wrong_lower, correct in case_insensitive_dict.items():
            if self.should_correct(wrong_lower, self.protection_dict):
                # Check if the term is an English word (contains only ASCII letters)
                is_english_word = all(c.isalpha() and ord(c) < 128 for c in wrong_lower.strip())

                if is_english_word:
                    # For English words, add word boundary markers
                    pattern = re.compile(r'\b' + re.escape(wrong_lower) + r'\b', re.IGNORECASE)
                else:
                    # For non-English terms (like Chinese), use simple pattern
                    pattern = re.compile(re.escape(wrong_lower), re.IGNORECASE)

                patterns[wrong_lower] = (pattern, correct)

        # Sort correction items by length (longest first) for proper matching
        sorted_correction_items = sorted(
            [(wrong_lower, correct, pattern) for wrong_lower, (pattern, correct) in patterns.items()],
        ...
        for chunk_start in range(0, total_lines, chunk_size):
            ...
            for i, line in enumerate(chunk):
                ...
                for wrong_lower, correct, pattern in sorted_correction_items:
                    def replace_func(match):
                        ...
                        # Always use the correct replacement regardless of case
                        replaced = correct
                    ...
                    if count > 0:
                        line = new_line
                        replacements[(wrong_lower, correct)] += count
